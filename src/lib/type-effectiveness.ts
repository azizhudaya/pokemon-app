...
/**
 * Multiplier lookup: TYPE_CHART[attackingType][defendingType] → number
 *
 * Values: 0 (immune), 0.5 (not very effective), 1 (neutral), 2 (super effective).
 * Derived from the Gen 6+ canonical chart (Fairy introduced; unchanged through Gen 9).
 *
 * Every row and column is expressed fully to avoid silent missing-key bugs.
 */
...
/**
 * Compute the raw dual-type multiplier a defender takes from an attacking type,
 * ignoring abilities. A single-typed defender passes one type; return type
...
export function getTypeMultiplier(
  ...
  return defendingTypes.reduce(
    (accumulated, defendingType) => accumulated * row[defendingType],
    1,
  );
}

/**
 * Mutate the multiplier to reflect defender abilities that grant immunity or
 * resistance (Levitate, Flash Fire, etc.). Tera Type handling is done by the
 * caller: it replaces the defender's types before reaching this function.
 */
export function applyAbilityOverrides(
  multiplier: number,
  attackingType: PokemonType,
  ability: string | null,
): number {
  ...
  const IMMUNITY_BY_ABILITY: Record<string, PokemonType> = {
    levitate: "ground",
    "flash fire": "fire",
    "water absorb": "water",
    "dry skin": "water",
  ...
  if (IMMUNITY_BY_ABILITY[normalized] === attackingType) {
    return 0;
  }

  if (normalized === "thick fat" && (attackingType === "fire" || attackingType === "ice")) {
    return multiplier * 0.5;
  }

  if (normalized === "heatproof" && attackingType === "fire") {
    return multiplier * 0.5;
  }

  if (normalized === "dry skin" && attackingType === "fire") {
